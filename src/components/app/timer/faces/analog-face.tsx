...
export function AnalogFace({ time, subjectName, mode, totalDuration }: AnalogFaceProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center relative">
        ...
        <div className="absolute w-full h-full">
             {Array.from({ length: 12 }).map((_, i) => (
                <div key={i} className="absolute w-full h-full" style={{ transform: `rotate(${i * 30}deg)` }}>
                    <div className="absolute top-[5%] left-1/2 -translate-x-1/2 h-2 w-[2px] bg-muted rounded-full"></div>
                </div>
            ))}
        </div>
        ...
        >
            <div className="w-full h-[60%] bg-primary rounded-full"></div>
        </motion.div>
        ...
        <div className="absolute bottom-[20%] text-center">
            <span className="font-mono font-bold text-4xl text-foreground tracking-tighter">
                {formatTime(time)}
            </span>
